      ...
      <el-col :span="24">
        <el-card>
          ...
          <el-table
            ...
            stripe
            :data="pvsList.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase()))"
            width="100%"
...
<script>
import {getPVsList} from "@/api/k8s";
...
export default {
  ...
  methods: {
    async fetchData(query) {
      ...
      await getPVsList(query).then(res => {
        this.pvsList = res.data
      })
