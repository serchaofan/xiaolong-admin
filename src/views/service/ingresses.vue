      ...
      <el-col :span="24">
        <el-card>
          ...
          <el-table
            ...
            stripe
            :data="ingressesList.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase()))"
            width="100%"
...
<script>
    import {getIngressesList} from "@/api/k8s";
    ...
    export default {
      ...
      methods: {
        async fetchData(query) {
          ...
          await getIngressesList(query).then(res => {
            this.ingressesList = res.data
          })
