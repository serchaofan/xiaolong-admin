    ...
    <el-row :gutter="20">
      <el-col :span="24">
        <el-card>
          ...
          <el-table
            stripe
            :data="statefulsetsList.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase()))"
            width="100%"
...
<script>
    import {getStatefulsList} from "@/api/k8s";
    ...
    export default {
      ...
      methods: {
        async fetchData(query) {
          ...
          await getStatefulsList(query).then(res => {
            this.statefulsetsList = res.data
          })
      ...
      watch: {
        '$store.state.namespace.namespace': function () {
          ...
          console.log("statefulsets.vue", this.query)
          this.fetchData(this.query)
        }
      }
    }
